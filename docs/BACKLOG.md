...
### 🎟️ **TICKET: FALTMAP-40 - Fix Map Pin Visual Glitch During Progressive Geocoding**
- Epic: E03 (Testing & Reliability)
- Status: Open
- Priority: 🟡 High
...
1. **Staggered animation delay** - `updateMapMarkers` with `animate=true` adds markers with `setTimeout` delays
2. **Cluster animation conflicts** - `animateAddingMarkers: true` tries to animate positioning while markers are being added progressively
3. **Progressive one-by-one updates** - Each new geocoded restaurant triggers immediate marker addition, causing constant clustering recalculation

When zoomed out, the clustering algorithm temporarily positions markers at edges before final placement, creating the visible "jump" effect.

**Scope of Work:**

1. **Remove staggered animation during progressive geocoding:**
   - In `content.js` line 88, change `updateMapMarkers` call from `animate=true` to `animate=false`
   - This eliminates the `setTimeout` delay that causes positioning issues
   - Allows clustering algorithm to position correctly from the start

2. **Verify fix:**
   - Test with map zoomed out to various levels
   - Trigger geocoding for 20+ restaurants
   - Confirm pins appear directly at correct locations
   - Ensure no visual "jumping" or "edge appearance" behavior

3. **Ensure pulse animation still works:**
   - The `marker-pulse` CSS animation is independent of the staggered animation
   - Verify new markers still have visual highlight
   - Check `CONFIG.ANIMATION.MARKER_PULSE_MS` is applied correctly

**Files to Modify:**
- `content.js` (line 88: updateMapMarkers call)

**Technical Notes:**
- The `animate` parameter controls staggered `setTimeout` delays, not the pulse animation
- `animate=false` means "add all markers immediately without delay"
- Clustering will still work correctly, just without the artificial delay
- This is a one-line change with significant UX improvement

**Acceptance Criteria:**
- [ ] `updateMapMarkers` called with `animate=false` during progressive geocoding (line 88)
- [ ] No `setTimeout` delays when adding markers during geocoding
...
- [ ] Test with 20+ restaurants at various zoom levels (zoom 9, 10, 11, 12)
- [ ] Manual testing confirms smooth pin appearance
