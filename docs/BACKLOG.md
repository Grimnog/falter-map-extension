...
### 🎟️ **TICKET: FALTMAP-41 - Fix Modal Status Badge UI Issues**
- Epic: E04 (UI/UX Polish)
- Status: Open
- Priority: 🟡 High
...
**Context:**
The modal-status badge (showing "X/Y" geocoding progress) has several visual issues:
1. **Noisy animation** - Border pulses grey→black during geocoding, creating visual distraction
2. **Border too thin** - Border should be bolder/thicker for better definition
3. **Size instability** - Badge grows when counter reaches double digits (e.g., "10/22"), causing layout shift
4. **Dead code** - Unused progress-container HTML (progress-bar and progress-text) serves no purpose

**Scope of Work:**

1. **Remove pulsing animation:**
   - Find CSS causing border color pulse (grey→black)
   - Remove or disable the animation
   - Keep border consistently black

2. **Increase border thickness:**
   - Increase border width from current value to ~2px
   - Ensure good contrast and definition

3. **Fix size stability:**
   - Set fixed width on `.status-value` to prevent growth
   - Calculate width that fits "99/999" (maximum expected)
   - Use `min-width` or fixed `width` to prevent layout shift

4. **Remove unused HTML:**
   - Delete `.progress-container` div and children from modal HTML
   - Remove associated CSS if any
   - Clean up any JavaScript references to progress-bar/progress-text IDs

**Files to Modify:**
- `modules/MapModal.js` (lines 72-75: remove progress-container HTML)
- `content.css` (modal-status styles, border, animations, sizing)
- Verify no JS references to removed elements

**Technical Notes:**
- The status badge shows "X/Y" format where X and Y can be 1-3 digits
- Need to accommodate up to "999/999" worst case
- Border pulse might be a loading animation class
- Check for `.loading` class or keyframe animations

**Acceptance Criteria:**
- [ ] Border does not pulse or change color during geocoding
- [ ] Border is consistently black and thicker (~2px)
- [ ] Status badge has fixed width that accommodates "99/999"
- [ ] No layout shift when counter reaches double/triple digits
- [ ] Progress-container HTML removed from MapModal.js
- [ ] Associated CSS removed (if any)
- [ ] No JavaScript errors from removed elements
- [ ] Visual appearance clean and stable during geocoding
